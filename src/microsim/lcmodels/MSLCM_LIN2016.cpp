...
// ===========================================================================
MSLCM_LIN2016::MSLCM_LIN2016(MSVehicle& v) :
    MSAbstractLaneChangeModel(v, LaneChangeModel::LIN2016),
...
void
MSLCM_LIN2016::initDerivedParameters() {
...
bool
MSLCM_LIN2016::debugVehicle() const {
    return DEBUG_COND;
}


int
MSLCM_LIN2016::wantsChangeSublane(
    int laneOffset,
...
void
MSLCM_LIN2016::setOwnState(const int state) {
...
void
MSLCM_LIN2016::updateSafeLatDist(const double travelledLatDist) {
...
double
MSLCM_LIN2016::patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
    gDebugFlag2 = DEBUG_COND;
...
double
MSLCM_LIN2016::_patchSpeed(double min, const double wanted, const double max, const MSCFModel& cfModel) {
    if (wanted <= 0) {
        return wanted;
    }
...
void*
MSLCM_LIN2016::inform(void* info, MSVehicle* sender) {
...
void
MSLCM_LIN2016::msg(const CLeaderDist& cld, double speed, int state) {
    assert(cld.first != 0);
...
double
MSLCM_LIN2016::informLeader(int blocked,
                           int dir,
...
void
MSLCM_LIN2016::informFollower(int blocked,
                             int dir,
...
double
MSLCM_LIN2016::informLeaders(int blocked, int dir,
...
void
MSLCM_LIN2016::informFollowers(int blocked, int dir,
                              const std::vector<CLeaderDist>& blockers,
                              double remainingSeconds,
                              double plannedSpeed) {
    // #3727
...
MSLCM_LIN2016::prepareStep() {
    ...
    if (myExpectedSublaneSpeeds.size() != myVehicle.getLane()->getEdge().getSubLaneSides().size()) {
        ...
        if (currEdge->canChangeToOpposite()) {
            MSLane* opposite = lanes.back()->getOpposite();
            const int subLanes = MAX2(1, int(ceil(opposite->getWidth() / MSGlobals::gLateralResolution)));
            for (int i = 0; i < subLanes; ++i) {
                newExpectedSpeeds.push_back(lanes.back()->getVehicleMaxSpeed(&myVehicle));
            }
        }
...
double
MSLCM_LIN2016::getPosLat() {
    return myVehicle.getLateralPositionOnLane() + mySigmaState;
}

int
MSLCM_LIN2016::computeSublaneShift(const MSEdge* prevEdge, const MSEdge* curEdge) {
...
int
MSLCM_LIN2016::_wantsChangeSublane(
    int laneOffset,
    LaneChangeAction alternatives,
    const MSLeaderDistanceInfo& leaders,
    const MSLeaderDistanceInfo& followers,
    ...
    */
    ...
    const double rightEnd = laneOffset == 0 ? leftMax : (checkOpposite ? getLeftBorder() : edge.getWidth());
#ifdef DEBUG_WANTSCHANGE
...
int
MSLCM_LIN2016::slowDownForBlocked(MSVehicle** blocked, int state) {
...
void MSLCM_LIN2016::addLCSpeedAdvice(const double vSafe) {
...
void
MSLCM_LIN2016::updateExpectedSublaneSpeeds(const MSLeaderDistanceInfo& ahead, int sublaneOffset, int laneIndex) {
...
double
MSLCM_LIN2016::forecastAverageSpeed(double vSafe, double vMax, double gap, double vLeader) const {
...
double
MSLCM_LIN2016::computeSpeedGain(double latDistSublane, double defaultNextSpeed) const {
    ...
    for (int i = 0; i < (int)sublaneSides.size(); ++i) {
        const double leftSide = i + 1 < (int)sublaneSides.size() ? sublaneSides[i + 1] : MAX2(myVehicle.getLane()->getEdge().getWidth(), sublaneSides[i] + POSITION_EPS);
...
CLeaderDist
MSLCM_LIN2016::getLongest(const MSLeaderDistanceInfo& ldi) const {
    int iMax = -1;
    double maxLength = -1;
...
bool
MSLCM_LIN2016::tieBrakeLeader(const MSVehicle* veh) const {
...
CLeaderDist
MSLCM_LIN2016::getSlowest(const MSLeaderDistanceInfo& ldi) {
    int iMax = 0;
...
int
MSLCM_LIN2016::checkBlocking(const MSLane& neighLane, double& latDist, double maneuverDist, int laneOffset,
                            const MSLeaderDistanceInfo& leaders,
                            const MSLeaderDistanceInfo& followers,
                            const MSLeaderDistanceInfo& /*blockers */,
                            const MSLeaderDistanceInfo& neighLeaders,
...
int
MSLCM_LIN2016::checkBlockingVehicles(
...
void
MSLCM_LIN2016::updateCFRelated(const MSLeaderDistanceInfo& vehicles, double foeOffset, bool leaders) {
...
bool
MSLCM_LIN2016::overlap(double right, double left, double right2, double left2) {
    assert(right <= left);
    assert(right2 <= left2);
...
int
MSLCM_LIN2016::lowest_bit(int changeReason) {
...
MSLCM_LIN2016::StateAndDist
MSLCM_LIN2016::decideDirection(StateAndDist sd1, StateAndDist sd2) const {
...
LaneChangeAction
MSLCM_LIN2016::getLCA(int state, double latDist) {
...
int
MSLCM_LIN2016::checkStrategicChange(int ret,
                                   int laneOffset,
                                   const MSLeaderDistanceInfo& leaders,
                                   const MSLeaderDistanceInfo& neighLeaders,
...
double
MSLCM_LIN2016::computeGapFactor(int state) const {
    return (state & LCA_STRATEGIC) != 0 ? MAX2(0.0, (1.0 - myPushy * (1 + 0.5 * myImpatience))) : 1.0;
}


int
MSLCM_LIN2016::keepLatGap(int state,
                         const MSLeaderDistanceInfo& leaders,
                         const MSLeaderDistanceInfo& followers,
                         const MSLeaderDistanceInfo& blockers,
                         const MSLeaderDistanceInfo& neighLeaders,
...
void
MSLCM_LIN2016::updateGaps(const MSLeaderDistanceInfo& others, double foeOffset, double oldCenter, double gapFactor,
...
double
MSLCM_LIN2016::getWidth() const {
    return myVehicle.getVehicleType().getWidth() + NUMERICAL_EPS;
}


double
MSLCM_LIN2016::computeSpeedLat(double latDist, double& maneuverDist, bool urgent) const {
...
void
MSLCM_LIN2016::commitManoeuvre(int blocked, int blockedFully,
                              const MSLeaderDistanceInfo& leaders,
                              const MSLeaderDistanceInfo& neighLeaders,
                              const MSLane& neighLane,
                              double maneuverDist) {
...
double
MSLCM_LIN2016::commitFollowSpeed(double speed, double latDist, double secondsToLeaveLane, const MSLeaderDistanceInfo& leaders, double foeOffset) const {
...
double
MSLCM_LIN2016::getSafetyFactor() const {
    return 1 / ((1 + 0.5 * myImpatience) * myAssertive);
}

double
MSLCM_LIN2016::getOppositeSafetyFactor() const {
    return myOppositeParam <= 0 ? std::numeric_limits<double>::max() : 1 / myOppositeParam;
}


std::string
MSLCM_LIN2016::getParameter(const std::string& key) const {
...
void
MSLCM_LIN2016::setParameter(const std::string& key, const std::string& value) {
    double doubleValue;
    try {
...
double
MSLCM_LIN2016::getLeftBorder(bool checkOpposite) const {
...
double
MSLCM_LIN2016::getVehicleCenter() const {
    if (isOpposite()) {
...
double
MSLCM_LIN2016::getNeighRight(const MSLane& neighLane) const {
    if (isOpposite()) {
...
bool
MSLCM_LIN2016::preventSliding(double maneuverDist) const {
...
bool
MSLCM_LIN2016::wantsKeepRight(double keepRightProb) const {
    return keepRightProb * myKeepRightParam > MAX2(myChangeProbThresholdRight, mySpeedGainProbabilityLeft);
}


double
MSLCM_LIN2016::saveBlockerLength(double length, double foeLeftSpace) {
